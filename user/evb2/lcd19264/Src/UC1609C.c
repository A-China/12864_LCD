		...
		if ( (uint8_t)*str >= 0xC0 ){	// code 0xC0 corresponds to the Cyrillic character 'A' according to ASCII Win-1251
			...
			switch ((uint8_t)*str) {
				case 0xD0: {
					...
					if ((uint8_t)*str == 0x81) { buff_char = 0xA8; break; }		// byte of the Ё character (if more characters are needed, add here and in the DrawChar () function)
				...
				case 0xD1: {
					...
					str++;
					// check the second byte there the character itself
					if ((uint8_t)*str == 0x91) { buff_char = 0xB8; break; }		// byte of character ё (if more characters are needed, add here and in the DrawChar () function)
